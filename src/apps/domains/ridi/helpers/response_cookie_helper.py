...
class ResponseCookieHelper:
    ...
    def clear_token_cookie(cls, response, root_domain: str):
        cls.clear_cookie(response=response, key=ACCESS_TOKEN_COOKIE_KEY, domain=root_domain)
        cls.clear_cookie(response=response, key=REFRESH_TOKEN_COOKIE_KEY, domain=root_domain)

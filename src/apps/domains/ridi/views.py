...
from lib.decorators.cookie_handler import add_tokens_in_cookie, clear_tokens_in_cookie, EMPTY_TOKENS
...
class CallbackView(View):
    ...
    def get(self, request):
        ...
        if deprecated:
            tokens = TokenRequestHelper.get_tokens(
        ...
        tokens = AuthorizationCodeService.get_tokens(
...
class TokenView(APIView):
    ...
    def post(self, request):
        ...
        if not access_token:
            tokens = TokenRefreshService.get_tokens(valid_data['refresh_token'])
            data = TokenHelper.get_token_data_info(tokens[0])

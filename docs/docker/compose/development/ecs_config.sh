...
ecs-cli configure --region ap-northeast-2 --cluster account-fixed-cluster
...
ecs-cli compose --project-name account-scalable --file docker-compose-www.yml service up \
--deployment-min-healthy-percent 100 \
--deployment-max-percent 200 \
